...
bun_result_t bun_parse_header(BunParseContext *ctx, BunHeader *header) {
  ...
  if (ctx->file_size < (long)BUN_HEADER_SIZE) {
    bun_log_error(ctx,
                  "Header claims smaller filesize than actual header size.");
  ...
  if (header->magic != BUN_MAGIC) {
    bun_log_error(ctx,
                  "Malformed: magic number does not match (expected: 0x%08X, "
                  "got: 0x%08X)",
  ...
  if (header->version_major != BUN_VERSION_MAJOR ||
    ...
    bun_log_error(
        ctx, "Unsupported: version not supported (expected: 1/0, got: %u.%u)",
  ...
  if (header->asset_table_offset % 4 != 0) {
    bun_log_error(
        ctx, "Malformed: asset_table_offset not divisible by 4 (got: %llu)",
        header->asset_table_offset);
    exit_code = BUN_MALFORMED;
  }
  if (header->string_table_offset % 4 != 0) {
    bun_log_error(
        ctx, "Malformed: string_table_offset not divisible by 4 (got: %llu)",
        header->string_table_offset);
    exit_code = BUN_MALFORMED;
  }
  if (header->data_section_offset % 4 != 0) {
    bun_log_error(
        ctx, "Malformed: data_section_offset not divisible by 4 (got: %llu)",
  ...
  if (header->string_table_size % 4 != 0) {
    bun_log_error(ctx,
                  "Malformed: string_table_size not divisible by 4 (got: %llu)",
                  header->string_table_size);
    exit_code = BUN_MALFORMED;
  }
  if (header->data_section_size % 4 != 0) {
    bun_log_error(ctx,
                  "Malformed: data_section_size not divisible by 4 (got: %llu)",
                  header->data_section_size);
    exit_code = BUN_MALFORMED;
  }

  // Notes 9,  2: All sections must, in their entirety, remain within the bounds
  // of the file. If the declared start or end of a section falls outside the
  // bounds of the containing file, that is a parse error.
  ...
  if (header->asset_table_offset > file_size - asset_table_size ||
    ...
    bun_log_error(
        ctx,
        "Malformed file: section exceeds file bounds (asset: %llu-%llu, "
        "string: %llu-%llu, data: %llu-%llu, file_size: %llu)",
        header->asset_table_offset,
